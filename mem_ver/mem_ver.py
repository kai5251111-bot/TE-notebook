...
def run_internal_staircase(addr="GPIB0::22::INSTR", start_v=1, stop_v=125, step=1, dwell_s=0.01, nplc=0.05):
...
# 儲存掃描設定到暫存器
# def save_staircase(addr, start_v, stop_v, step, dwell_s, nplc, store_id):
#     with Keithley2400(addr) as smu:
...
#         # 儲存到暫存器
#         smu.write(f"*SAV {store_id}")
#         print(f"設定已儲存到暫存 {store_id}，範圍 {start_v}~{stop_v}V，步進 {step}V")


# # 從暫存器執行掃描並回傳 DataFrame (含 V_cmd 和 segment)
# def run_from_store(addr, store_id, start_v, stop_v, step, segment_name):
#     with Keithley2400(addr) as smu:
#         points = int(math.floor((stop_v - start_v) / step) + 1)
...
#             smu.reset()
#             smu.write("TRAC:CLE")
#             smu.write(f"*RCL {store_id}")
#             # points = int(smu.ask("TRIG:COUN?"))  # 讀取暫存設定的點數
#             smu.write(f"TRAC:POIN {points}")
#             smu.write("TRAC:FEED SENS")
#             smu.write("TRAC:FEED:CONT NEXT")
#             smu.write(f"TRIG:COUN {points}")
#             smu.write("TRIG:SOUR IMM")
#             smu.write("OUTP:ZERO:STAT ON")
...
# # 主流程：儲存兩段設定並執行，合併結果
# def run_two_segments(addr="GPIB0::22::INSTR"):
#     start_time = time.time()
...
#     # 執行兩段掃描
#     df1 = run_from_store(addr, store_id=1, start_v=10, stop_v=90, step=10, segment_name="Segment1")
#     df2 = run_from_store(addr, store_id=2, start_v=91, stop_v=125, step=1, segment_name="Segment2")
...
if __name__ == "__main__":
    addr="GPIB0::22::INSTR"
    # save_staircase(addr, start_v=10, stop_v=90, step=10, dwell_s=0.01, nplc=0.05, store_id=1)
    # save_staircase(addr, start_v=91, stop_v=125, step=1, dwell_s=0.01, nplc=0.05, store_id=2)
    root = tk.Tk()
